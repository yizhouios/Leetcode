...
class Solution874 {
    ...
    func robotSim_new(_ commands: [Int], _ obstacles: [[Int]]) -> Int {
    ...
    func robotSim(_ commands: [Int], _ obstacles: [[Int]]) -> Int {
        enum Direction: Int {
            case north = 0, east = 1, sourth = 2, west = 3
        }
        var obstaclesDict = [String: Optional<Any>]()
        for obstacle in obstacles {
            obstaclesDict["#\(obstacle[0])#\(obstacle[1])#"] = true // 添加#号区分，避免值判断出错
        }
        var cur: [Int] = [0, 0]
        var ans = 0
        var orientaton = Direction.north
        for var i in commands {
            if i == -1 {
                orientaton = Direction(rawValue: (orientaton.rawValue + 1) % 4)!
            } else if i == -2 {
                orientaton = Direction(rawValue: (orientaton.rawValue + 3) % 4)!
            } else {
                while i > 0 {
                    i -= 1
                    var tmpAns = cur
                    ...
                    if obstaclesDict[String("#\(tmpAns[0])#\(tmpAns[1])#")] == nil {
                        cur = tmpAns
                        ans = max(ans, abs(Int(pow(Double(tmpAns[0]), 2.0) + pow(Double(tmpAns[1]), 2.0))))
                    } else {
                        break
                    }
                }
